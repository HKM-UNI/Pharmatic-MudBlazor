...
namespace Pharmatic.Services
{
    public class TypeDoseService
    {
        ...
        {
            var result = await _http.PostAsJsonAsync("http://localhost:7035/api/dosage_forms/create", new_dose);
        ...
        {
            var result = await _http.PatchAsJsonAsync($"http://localhost:7035/api/dosage_forms/{id}", dose);
            var response = await result.Content.ReadFromJsonAsync<TypeDoseDTO>();
            return response!;
        }
        ...
        {
            var url = $"http://localhost:7035/api/dosage_forms/{id}";
